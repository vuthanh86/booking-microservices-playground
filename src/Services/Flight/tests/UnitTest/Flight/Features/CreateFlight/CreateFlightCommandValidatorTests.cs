...
public class CreateFlightCommandValidatorTests
{
    ...
    {
        // Arrange
        CreateFlightCommand command = new FakeValidateCreateFlightCommand().Generate();
        CreateFlightCommandValidator validator = new CreateFlightCommandValidator();

        // Act
        TestValidationResult<CreateFlightCommand> result = validator.TestValidate(command);
