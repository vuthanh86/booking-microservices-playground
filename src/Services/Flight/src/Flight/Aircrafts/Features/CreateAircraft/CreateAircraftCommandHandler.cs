...
public class CreateAircraftCommandHandler : IRequestHandler<CreateAircraftCommand, AircraftResponseDto>
{
    ...
    public async Task<AircraftResponseDto> Handle(CreateAircraftCommand command, CancellationToken cancellationToken)
    {
        ...
        Aircraft aircraft
            = await _flightDbContext.Aircraft.SingleOrDefaultAsync(x => x.Model == command.Model, cancellationToken);
        ...
        EntityEntry<Aircraft> newAircraft = await _flightDbContext.Aircraft.AddAsync(aircraftEntity, cancellationToken);
