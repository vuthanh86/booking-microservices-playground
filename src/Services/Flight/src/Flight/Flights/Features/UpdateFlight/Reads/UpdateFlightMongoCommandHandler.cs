...
public class UpdateFlightMongoCommandHandler : ICommandHandler<UpdateFlightMongoCommand>
{
    ...
    public async Task<Unit> Handle(UpdateFlightMongoCommand command, CancellationToken cancellationToken)
    {
        ...
        FlightReadModel flightReadModel = _mapper.Map<FlightReadModel>(command);

        FlightReadModel flight = await _flightReadDbContext.Flight.AsQueryable()
                                     .FirstOrDefaultAsync(x => x.FlightId == flightReadModel.FlightId,
                                                          cancellationToken);
        ...
        await _flightReadDbContext.Flight.UpdateOneAsync(
                                                         ...
                                                         Builders<FlightReadModel>.Update
                                                             ...
                                                             .Set(x => x.DurationMinutes,
                                                                  flightReadModel.DurationMinutes)
                                                             ...
                                                             .Set(x => x.ArriveAirportId,
                                                                  flightReadModel.ArriveAirportId)
                                                             .Set(x => x.DepartureAirportId,
                                                                  flightReadModel.DepartureAirportId),
