...
public class CreateFlightCommandHandler : ICommandHandler<CreateFlightCommand, FlightResponseDto>
{
    ...
    public async Task<FlightResponseDto> Handle(CreateFlightCommand command, CancellationToken cancellationToken)
    {
        ...
        Models.Flight flightEntity = Models.Flight.Create(command.Id, command.FlightNumber, command.AircraftId,
                                                          command.DepartureAirportId, command.DepartureDate,
                                                          command.ArriveDate, command.ArriveAirportId,
                                                          command.DurationMinutes, command.FlightDate, command.Status,
                                                          command.Price);

        EntityEntry<Models.Flight> newFlight = await _flightDbContext.Flights.AddAsync(flightEntity, cancellationToken);
