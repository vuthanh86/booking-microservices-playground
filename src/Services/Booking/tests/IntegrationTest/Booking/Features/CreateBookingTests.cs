...
public class CreateBookingTests : IntegrationTestBase<Program, BookingDbContext>
{
    ...
    {
        // Arrange
        CreateBookingCommand command = new FakeCreateBookingCommand().Generate();

        // Act
        ulong response = await Fixture.SendAsync(command);
    ...
    private void MockFlightGrpcServices(IServiceCollection services)
    {
        services.Replace(ServiceDescriptor.Singleton(x =>
        {
            IFlightGrpcService mock = Substitute.For<IFlightGrpcService>();
            ...
            mock.GetAvailableSeats(Arg.Any<long>())
                .Returns(
                         new UnaryResult<IEnumerable<SeatResponseDto>>(Task.FromResult(FakeSeatsResponseDto
                                                                           .Generate())));
