...
public class Booking : AggregateEventSourcing<long>
{
    ...
    {
        Booking booking = new Booking { Id = id, Trip = trip, PassengerInfo = passengerInfo, IsDeleted = isDeleted };

        BookingCreatedDomainEvent @event
            = new BookingCreatedDomainEvent(booking.Id, booking.PassengerInfo, booking.Trip, booking.IsDeleted);
