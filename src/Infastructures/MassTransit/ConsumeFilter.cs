    ...
    where T : class
{
    ...
    {
        Guid id = await _persistMessageProcessor.AddReceivedMessageAsync(
                                                                         new MessageEnvelope(
                                                                          context.Message,
                                                                          context.Headers.ToDictionary(x => x.Key,
                                                                           x => x.Value))
                                                                        );

        PersistMessage message = await _persistMessageProcessor.ExistMessageAsync(id);
