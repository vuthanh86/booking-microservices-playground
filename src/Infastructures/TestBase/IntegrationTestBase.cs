    ...
    where TEntryPoint : class
{
                ...
                builder.ConfigureServices(services =>
                {
                    ...
                    services.AddMassTransitTestHarness(x =>
                    {
                        x.UsingRabbitMq((context, cfg) =>
                        {
                            ...
                            string host = rabbitMqOptions.HostName;
    ...
    {
        int time = timeoutSecond ?? Timeout;

        DateTime startTime = DateTime.Now;
        bool timeoutExpired = false;
        bool meet = await conditionToMet.Invoke();
        while (!meet)
        {
        ...
        where TInternalCommand : class, IInternalCommand
    {
        await WaitUntilConditionMet(async () =>
        {
            ...
            {
                IPersistMessageProcessor persistMessageProcessor = sp.GetService<IPersistMessageProcessor>();
                Guard.Against.Null(persistMessageProcessor, nameof(persistMessageProcessor));

                IReadOnlyList<PersistMessage> filter = await persistMessageProcessor.GetByFilterAsync(x =>
                                                           x.DeliveryType == MessageDeliveryType.Internal &&
                                                           TypeProvider.GetTypeName(typeof(TInternalCommand)) ==
                                                           x.DataType);

                bool res = filter.Any(x => x.MessageStatus == MessageStatus.Processed);
    ...
    {
        ITestHarness harness = ServiceProvider.GetTestHarness();
        return harness;
    }
    ...
    where TWContext : DbContext
{
    ...
    public Task InsertAsync<T>(params T[] entities) where T : class
    {
        ...
        {
            foreach (T entity in entities) db.Set<T>().Add(entity);
    ...
    where TEntryPoint : class
{
    ...
    public async Task InitializeAsync()
    {
        ...
        IOptions<MongoOptions> mongoOptions = Fixture.ServiceProvider.GetRequiredService<IOptions<MongoOptions>>();
...
public abstract class IntegrationTestBase<TEntryPoint, TWContext, TRContext> : IntegrationTestFixtureCore<TEntryPoint>,
                                                                               IClassFixture<IntegrationTestFixture<
                                                                                   TEntryPoint, TWContext, TRContext>>
    where TEntryPoint : class
    where TWContext : DbContext
    where TRContext : MongoDbContext
{
